...
const Register = () => {
  ...
  const RegisterSchema = Yup.object().shape({
    fullName: Yup.string()
      .min(
        fullNameLength.min,
        `Fullname must contain at least ${fullNameLength.min} characters`
      )
      .max(
        fullNameLength.max,
        `Fullname must contain ${fullNameLength.max} characters at most`
      )
    ...
    password: Yup.string()
      .min(
        passwordLength.min,
        `Password length must be ${passwordLength.min} or more characters`
      )
      .max(
        passwordLength.max,
        `Password shold not be more than ${passwordLength.max} characters`
      )
        ...
        <div>
          ...
          <Form id="formInput" method="POST" onSubmit={formik.handleSubmit}>
            ...
            <div className={loginStyle.button}>
              <Button
                className={`button ${loginStyle.signUp}`}
                size="lg"
                type="submit"
                disabled={isLoading}
              >
          ...
          <div className={loginStyle.button}>
            <Button
              className={`button ${loginStyle.signIn}`}
