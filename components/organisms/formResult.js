...
const SearchResult = ({ data, origin, destination, class_category, departure, child, adult }) => {
	return (
					...
					<div className={style.header}>
						<div className="row mt-3 m-2">
							...
							<div className="col-6 d-flex justify-content-end">
								<input type="date" className="form-control" name="" value={departure} />
							</div>
						</div>
					...
					<div className={`${style.container}`}>
						<div className="row mt-4">
							<div className={style.mainconten}>
								<h6 className={`text-secondary ${style.ticket}`}>{data?.length} flight found</h6>
							</div>
							{!data?.length ? (
								<>
									<div className="row justify-content-center mt-3">
										<div className="col-7 text-center">
											<Image src={logoNotFound} height={500} width={500} alt="logo not found" />
											<p className="text-secondary">Sorry, there is no flight schedule that you are looking for</p>
										</div>
									</div>
								</>
							) : (
								data.map((item, index) => (
									<>
										<Link href={`/detail-flight?id=${item.ticket_id}&child=${child}&adult=${adult}`} passHref>
												...
												<div className="row">
													...
													<div className="col-9">
														<div className="row">
															<div className="col-4">
																<div>
																	<h2 className={style.departureCity}>{item.origin}</h2>
																</div>
															</div>
															...
															<div className="col-5">
																<div className="ms-1">
																	<h2 className={style.arrivalCity}>{item.destination}</h2>
																</div>
															</div>
															<div className="col-8">
																<h6 className="text-secondary">{moment(item.departure).format("YYYY-MM-DD")}</h6>
															</div>
															<div className="col-4">
																<h6
																	style={{
